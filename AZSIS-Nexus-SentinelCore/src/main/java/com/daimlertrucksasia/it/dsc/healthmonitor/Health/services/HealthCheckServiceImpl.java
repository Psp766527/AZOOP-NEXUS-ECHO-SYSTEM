package com.daimlertrucksasia.it.dsc.healthmonitor.Health.services;
...
@Override
public Status checkHealthRest(Map<String, ?> parameters) {
    HealthCheckService.Status overall = HealthCheckService.Status.HEALTHY;
    ...
    for (HealthCheck check : checks) {
        ...
        if (!check.isPerformCheck(readOnly)) {
            log.info("Skipping health check for restapi: {}", name);
            continue;
        }
        ...
        try {
            ...
            if (!healthy && overall != HealthCheckService.Status.FAILED) {
                overall = HealthCheckService.Status.UNHEALTHY;     // don’t override FAILED
                log.warn("Health check UNHEALTHY: {}", name);
            } else if (healthy) {
                log.info("Health check passed: {}", name);
            }
        } catch (Exception ex) {
            log.error("Health check FAILED: {}", name, ex);
            statuses.put(check, false);
            overall = HealthCheckService.Status.FAILED;            // “sticky” highest severity
        }
    }
    return overall;
}
